...
export default function ApiKeyModal({ agent, onClose }) {
  ...
  return (
    <div className="fixed inset-0 bg-bg-deep/85 backdrop-blur-sm flex items-center justify-center z-[100] p-4">
      <div className="glass-panel w-full max-w-[560px] p-8" style={{ background: 'rgba(10, 17, 40, 0.95)', border: '1px solid var(--color-border-accent)' }}>
        <div className="flex justify-between items-center mb-6">
          <div className="flex items-center gap-3">
            <div className="p-2 rounded-[10px] bg-cyan-glow/15 border border-border-accent">
              <Key size={20} className="text-cyan-glow" />
            </div>
            <div>
          ...
          <button onClick={onClose} className="text-text-muted hover:text-white bg-transparent border-none cursor-pointer">
            <X size={20} />
          </button>
        </div>
        ...
        {!issuedKey ? (
          <div>
            <div className="mb-5">
              ...
              <div className="flex flex-wrap gap-2">
                {agent.allowed_scopes?.map((scope, idx) => (
                  <span
                    key={idx}
                    className="font-mono text-sm px-3 py-1.5 rounded-md bg-cyan-glow/12 text-cyan-glow border border-cyan-glow/30 font-medium"
                  >
                    {scope}
                  </span>
                ))}
            ...
            <div className="mb-7">
              ...
              <select
                ...
                className="w-full p-3 rounded-lg bg-bg-deep border border-border-glass text-white text-sm"
              >
            ...
            <div className="bg-amber-500/12 border border-amber-500/30 p-3.5 rounded-lg flex gap-3 items-start mb-7">
          ...
          <div>
            <div className="bg-emerald-500/15 border border-emerald-500 p-4 rounded-lg mb-6">
            ...
            <div className="mb-6">
              ...
              <div className="flex gap-2">
                <input
                  ...
                  className="font-mono flex-1 p-3 rounded-lg bg-bg-deep border border-border-accent text-cyan-glow text-sm"
                />
            ...
            <div className="grid grid-cols-2 gap-4 bg-bg-deep/70 p-4 rounded-lg mb-7">
              <div>
                <span className="text-xs text-text-muted uppercase">Key ID</span>
                <div className="font-mono text-sm text-white font-semibold">{issuedKey.key_id}</div>
              </div>
              <div>
                <span className="text-xs text-text-muted uppercase">Key Hint</span>
                <div className="font-mono text-sm text-white font-semibold">••••{issuedKey.key_hint}</div>
              </div>
            </div>
